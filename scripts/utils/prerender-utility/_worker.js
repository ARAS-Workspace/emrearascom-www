...
// ─────────────────────────────────────────────────────────────────────────────
// emrearas.com edge worker. Every request flows through six stages, in order:
//
 ...
 * images and svg there, never these), so that pattern is a safe immutable
 * signal; anything else revalidates. Widen the extension group only together
 * with everything that mirrors this contract in the build tooling.
...
export default {
  async fetch(request, env) {
    ...
    if (pathname.includes('.') && !pathname.endsWith('.html')) {
      ...
      // asset's Cache-Control once pinned HTML under a hashed .js URL as
      // immutable for a year — the loader MIME-rejected it on every later
      // load (the white "Hata" page). `no-store` because a miss must not be
      // remembered: the file may exist one deploy later. A 200/206/304 for a
      // real file passes through untouched — revalidations and range requests
