...
async function run() {
  ...
  await test('default document is Turkish, white, no-cache, Vary set', async () => {
    const r = await probe('/');
  ...
  await test('?locale=en switches the document', async () => {
    const r = await probe('/?locale=en');
  ...
  await test('preferred_locale cookie switches the document', async () => {
    const r = await probe('/', { cookie: 'preferred_locale=en' });
    expect(r.body.includes('lang="en"'), 'lang is not en');
  });

  await test('explicit query outranks the cookie', async () => {
    const r = await probe('/?locale=tr', { cookie: 'preferred_locale=en' });
    expect(r.body.includes('lang="tr"'), 'query did not win');
  });

  await test('unknown query locale falls through to the cookie', async () => {
    const r = await probe('/?locale=de', { cookie: 'preferred_locale=en' });
    expect(r.body.includes('lang="en"'), 'did not fall to cookie');
  });

  await test('unknown query locale without a cookie falls to the default', async () => {
    const r = await probe('/?locale=de');
    expect(r.body.includes('lang="tr"'), 'did not fall to default');
  });

  await test('malformed locale cookie falls to the default', async () => {
    const r = await probe('/', { cookie: 'preferred_locale=enx' });
    expect(r.body.includes('lang="tr"'), 'malformed cookie was honored');
  });
  ...
  await test('theme and locale combine', async () => {
    const r = await probe('/', { cookie: 'preferred_theme=g100; preferred_locale=en' });
    expect(r.body.includes('data-carbon-theme="g100"'), 'theme lost');
    expect(r.body.includes('lang="en"'), 'locale lost');
  });
  ...
  await test('/llms-full.txt?locale=en is a different document', async () => {
    const tr = await probe('/llms-full.txt');
    const en = await probe('/llms-full.txt?locale=en');
    expect(header(en, 'content-language') === 'en', `Content-Language ${header(en, 'content-language')}`);
    expect(sha(tr.body) !== sha(en.body), 'tr and en bodies are identical');
  });

  await test('the locale cookie reaches llms too', async () => {
    const r = await probe('/llms-full.txt', { cookie: 'preferred_locale=en' });
    expect(header(r, 'content-language') === 'en', `Content-Language ${header(r, 'content-language')}`);
  });
  ...
  await test('/ai/llms.txt serves the agent page dump', async () => {
    const r = await probe('/ai/llms.txt');
    expect(r.status === 200, `status ${r.status}`);
    expect(header(r, 'content-type').startsWith('text/markdown'), header(r, 'content-type'));
  });

  await test('a route without llms/ answers a real 404, not the shell', async () => {
    const r = await probe('/blog/llms.txt');
  ...
  await test('an unknown route follows the same 404 rule', async () => {
    const r = await probe('/olmayan/llms.txt');
    expect(r.status === 404, `status ${r.status}`);
  });
  ...
  await test('Accept: text/markdown turns a page into its llms text', async () => {
    const r = await probe('/privacy', { accept: 'text/markdown' });
    expect(r.status === 200, `status ${r.status}`);
    expect(header(r, 'content-type').startsWith('text/markdown'), header(r, 'content-type'));
  });

  await test('the markdown representation negotiates locale too', async () => {
    const r = await probe('/privacy', { accept: 'text/markdown', cookie: 'preferred_locale=en' });
    expect(header(r, 'content-language') === 'en', `Content-Language ${header(r, 'content-language')}`);
  });

  await test('Accept: text/html keeps the page HTML', async () => {
    const r = await probe('/privacy', { accept: 'text/html' });
  ...
  await test('Accept: */* means no preference — HTML stays primary', async () => {
    const r = await probe('/privacy', { accept: '*/*' });
    expect(header(r, 'content-type').startsWith('text/html'), header(r, 'content-type'));
  });

  await test('a route without llms/ falls through to HTML instead of 406', async () => {
    const r = await probe('/blog', { accept: 'text/markdown' });
    expect(r.status === 200, `status ${r.status}`);
    expect(header(r, 'content-type').startsWith('text/html'), header(r, 'content-type'));
  });
  ...
  await test('a trailing slash is the same page', async () => {
    const r = await probe('/blog/');
    expect(r.status === 200, `status ${r.status}`);
    expect(/<title>Blog/.test(r.body), 'did not serve the Blog document');
  });

  await test('the full matrix resolves: g100 + en on a subpage', async () => {
    const r = await probe('/privacy', { cookie: 'preferred_theme=g100; preferred_locale=en' });
    expect(r.body.includes('data-carbon-theme="g100"'), 'theme lost');
    expect(r.body.includes('lang="en"'), 'locale lost');
    expect(header(r, 'content-language') === 'en', `Content-Language ${header(r, 'content-language')}`);
  });
  ...
  await test('an unknown route is the shell, 200, revalidated', async () => {
    const r = await probe('/tamamen-olmayan-bir-sayfa');
    expect(r.status === 200, `status ${r.status}`);
    expect(header(r, 'content-type').startsWith('text/html'), header(r, 'content-type'));
    expect(header(r, 'cache-control') === 'no-cache', `Cache-Control ${header(r, 'cache-control')}`);
